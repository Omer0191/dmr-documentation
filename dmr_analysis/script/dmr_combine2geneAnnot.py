...
from .script_high.combine_annota2dmrs import merge_multiple_files2df, find_genome_chromSegment4dmr, \
    find_parallel_process_and_index, call_parallel_genome_function
...
def my_parser(parser):
    ...
    optional.add_argument('-inChSgFolder', '--dmr_chromSegmentFile_folder', default='chromSegment/', metavar='',
                          type=str,
    ...
    optional.add_argument('-numPro', '--number_of_processes', default=10, metavar='', type=int,
                          help='Number of parallel processes will be used to combine results , default=10')

    optional.add_argument('-inMinLen', '--miniLength_of_intergenetic', default=10, metavar='', type=int,
                          help='Minimum length of intergenetic regions will be considered, default=10')

    optional.add_argument('-inMinMeg', '--miniLength_of_merge', default=10, metavar='', type=int,
                          help='Minimum length will be merged by "bedtools merger", default=10')

    optional.add_argument('-inMinLogRegP', '--miniLogReg_proba_cutoff', default=0.8, metavar='', type=float,
                          help='minimym cutoff value of logReg_proba that will be used to select DMR for gene annotation, default = 0.8')

    return parser


def main(num_of_processes, logReg_proba_min_cutoff, in_dmr_sorted_df, all_in_genome_df, all_in_chromSegment_df,
         in_dmr_sorted_files):
    ...
    time.sleep(10)
    all_out_df = call_parallel_genome_function(num_of_processes, selected_dmr_sorted_df0, block_chunks,
                                               all_in_genome_df, all_in_chromSegment_df, in_dmr_sorted_files,
                                               logReg_proba_min_cutoff)
...
def run(args):
    ...
    tmp_in_genome_files = glob.glob(
        os.path.join(in_out_genome_annot_folder, args.dmr_genomeFile_folder, in_out_genome_file))
    ...
    all_in_genome_df.columns = ['chrs', 'start_pos', 'end_pos', 'genome_info', 'mr_chrs', 'mr_start_pos', 'mr_end_pos',
                                'mr_sites', 'mr_logReg_proba']
    ...
    all_in_chromSegment_df.columns = ['chrs', 'start_pos', 'end_pos', 'genome_info', 'mr_chrs', 'mr_start_pos',
                                      'mr_end_pos', 'mr_sites', 'mr_logReg_proba']

    # do parallel in genome annotation
    num_of_processes = args.number_of_processes
    logReg_proba_min_cutoff = args.miniLogReg_proba_cutoff
    main(num_of_processes, logReg_proba_min_cutoff, in_dmr_sorted_df, all_in_genome_df, all_in_chromSegment_df,
         in_dmr_sorted_files)
