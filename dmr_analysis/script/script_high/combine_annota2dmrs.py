...
def call_parallel_genome_function(num_of_processes, selected_dmr_sorted_df0, block_chunks, all_in_genome_df,
                                  all_in_chromSegment_df, in_dmr_sorted_files, logReg_proba_cutoff):
    # useing parallel computation for calling genome info function
    pool = mp.Pool(processes=num_of_processes, initializer=initpool,
                   initargs=(selected_dmr_sorted_df0, all_in_genome_df, all_in_chromSegment_df))
    ...
    files = pool.map(find_genome_chromSegment4dmr, [(block_chunks[loop], loop) for loop in range(0, num_of_processes)],
                     1)
    pool.close()
    ...
    out_file = in_dmr_sorted_files.replace('.bed',
                                           '_genome_chromSegment_info_ProbaGt_' + str(logReg_proba_cutoff) + '.bed')
...
if __name__ == '__main__':
    ...
    in_dmr_sorted_files = '24_chroms_high_miniPercentChange_gt_0.0001_Pcutoff_0.05_isSmooth_' + str(isSmooth) + \
                          '_isModTest_0__all_dmrRanking_top_' + str(top_percent) + '_minLogReg_proba_' + str(
        dmr_mini_cutoff) + '.bed'
