...
def my_parser(parser):
    ...
    required.add_argument('-inFolder', '--in_chromatinSegment_file_folder', required=True,
                          help='Path of input file folder for combined chromation segment files from six cells in bed format,'
                               ' combined six cells *.bed.gz files download from https://genome.ucsc.edu/cgi-bin/hgFileUi?db=hg19&g=wgEncodeAwgSegmentation ')
    required.add_argument('-outFolder', '--in_outFile_folder', required=True,
                          help='Path of output file folder for DMR/MRs mapped to chromSegments ')
    ...
    optional.add_argument('-inMinLen', '--in_minimum_length4region', default=10, type=int,
                          help='minimum length for each selected chromatin segment ,default= 10 bp')
    optional.add_argument('-inFileStr', '--in_fileName_string', default='*.bed.gz', type=str,
                          help='String name for input chromatin segment files that are stored in folder, default is *.bed.gz')
    optional.add_argument('-OFileStr', '--in_outFileName_string', default='combined_six_cells_chromatin_segment_min',
                          type=str, help='Output file type string '
                                         'that will be used for output combined chromatin segment files, default is "combined_six_cells_chromatin_segment_min"')
    optional.add_argument('-inMinOp', '--in_minimum_overlap4bedtools', default=1e-9, type=float,
                          help='minimmum overlap rate in bedtools intersection, default is 1e-9 or 1bp')
    optional.add_argument('-inExist', '--in_combined_chromatinSegment_exist', default=0, type=int,
                          help='Whether the combined six cell chromatin segment files exists or not, default=0 for not exist '
    ...
    optional.add_argument('-dmrCutoff', '--dmr_min_cutoff', default=0, metavar='', type=float,
                          help='minimum cutoff value for selecting DMR, default is 0 which means the cutoff value will be' \
...
def main(type_names, out_file_string, out_file_names, min_length, out_folder, column_name, in_sorted_dmr_file,
         dmr_min_cutoff, min_overlap, is_greater=True):
...
def run(args):
    ...
    if is_combined_chromatinSegment_exist == 0:
        ...
        print(in_files)
        out_file_names, type_names = read_bed_file_and_export2type_bed_file(in_files, column_name, out_folder,
                                                                            min_length)
        main(type_names, out_file_string, out_file_names, min_length, out_folder, column_name, in_sorted_dmr_file,
             dmr_min_cutoff, min_overlap, is_greater)
    else:
